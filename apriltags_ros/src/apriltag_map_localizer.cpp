...
#include <string>
/** utility function to debug transforms */
...
/** Callback function on tag_detections. 
    Publish another tag_detections_mapped with tag coords in relation to map
*/
...
//convert coords of tag in camera coords to map coords
...
//rebroadcast camera_to_map transform
  tf::TransformBroadcaster br;
  br.sendTransform(tf::StampedTransform(camera_to_map, time_now, 
        map, camera));
...
int main(int argc, char **argv){
  ...
  nh.param("robot_tag_size", robot_tag_size, 0.1); 
  ...
  if(!(tl.waitForTransform(camera, robot_tag, ros::Time(0), timeout))) {
      ROS_ERROR("No transform from %s to %s. Check that the camera can see the the robot tag. Check that the camera can see the the robot tag.", robot_tag.c_str(), camera.c_str());
  }
  if(!(tl.waitForTransform(map, robot_base, ros::Time(0), timeout))) {
      ROS_ERROR("No transform from %s to %s", robot_base.c_str(), map.c_str());
  }

 //get transfrom from camera to map. Use tag placed on robot and robot's pose
 // camera -> robot_tag -> robot_base -> map
...
//transform from robottag to robotbase:
//robottag coords:(facing tag, x->right, y->up, z->toward you) 
//robotbase coords:(facing robot and tag, x->toward you, y->right, z->up)
  tf::Transform robottag_to_robotbase(
      tf::Matrix3x3(0, 0, 1, 1, 0, 0, 0, 1, 0),
      tf::Vector3(0, 0, robot_tag_size/2.0));
  
  tf::Transform camera_to_map = robotbase_to_map * (robottag_to_robotbase * camera_to_robottag);

//broadcast transform
  tf::TransformBroadcaster br;
  br.sendTransform(tf::StampedTransform(camera_to_map, ros::Time::now(), 
        map, camera));
